...
# ── 텍스트 유틸 ──────────────────────────────────────
def _center_text(draw: ImageDraw.ImageDraw, text: str, y: int,
...
# ── 섹터 태그 박스 ───────────────────────────────────
def _draw_sector_tag(draw: ImageDraw.ImageDraw, sector: str,
                     x: int, y: int) -> int:
    """섹터 태그를 그리고 박스 너비 반환."""
    ...
    draw.text((x + pad_x, y + pad_y), text, font=font, fill=COLOR_TAG)
    return bw
...
def gen_list_card(is_gainer: bool, stocks: list[dict],
    ...
    """
    stocks 각 항목: {name, change, close, sector}
    미디어 박스에 3종목을 세로로 나열.
    종목명 옆에 섹터 태그 표시.
    """
    ...
    for i, s in enumerate(stocks):
        ...
        # ── 섹터 태그 (종목명 바로 아래) ──
        tag_y = name_y + 66
        _draw_sector_tag(draw, s.get("sector", "기타"), PAD, tag_y)

        # ── 등락률 (우측 상단) ──
        pct_text = f"{sign}{s['change']}%"
        bbox = draw.textbbox((0, 0), pct_text, font=f_pct)
        tw = bbox[2] - bbox[0]
        draw.text((W - PAD - tw, row_mid - 50), pct_text,
                  font=f_pct, fill=color)

        # ── 종가 (우측 하단) ──
        close_text = f"{s['close']:,}원"
        bbox2 = draw.textbbox((0, 0), close_text, font=f_close)
        tw2 = bbox2[2] - bbox2[0]
        draw.text((W - PAD - tw2, row_mid + 20), close_text,
                  font=f_close, fill=COLOR_GRAY)

        # ── 행 구분선 (마지막 행 제외) ──
        if i < config.TOP_N - 1:
            line_y = y_base + row_h - 1
...
def gen_chart_card(is_gainer: bool, stock_info: dict,
                   ...
                   out_path: Path) -> Path:
    """
    stock_info: {name, change, close, sector, ticker}
    자막 영역에 이유 텍스트, 미디어 박스에 matplotlib 차트.
    차트 제목 아래에 섹터 태그 표시.
    """
    bg     = config.GAINER_BG if is_gainer else config.LOSER_BG
    header = "📈  오늘의 급등주" if is_gainer else "📉  오늘의 급락주"
...
def _render_chart(stock_info: dict,
    ...
    ax.set_title(title_line, color=color, fontsize=20, pad=14, loc="left")
